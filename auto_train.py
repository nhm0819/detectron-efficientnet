...
# base
parser.add_argument('--data_path', type=str, default="", metavar='S',
                    help='absolute path of data path')
parser.add_argument('--file_storage_path', type=str, default="Z:\\private\\traning\\original", metavar='S',
                    help='absolute path of file storage')
parser.add_argument('--mask_path', type=str, default="Z:\\private\\traning\\original\\weights\\mask_rcnn_26_AP_98.33.pt", metavar='S',
                    help='pretrained mask rcnn X101 path')
parser.add_argument('--clf_path', type=str, default="Z:\\private\\traning\\original\\weights\\efficientnet_over_current_acc_98.55.pt", metavar='S',
...
parser.add_argument('--testing', action='store_true',
                    help='not make all image to mask image')


# mask rcnn
parser.add_argument('--mask_classes', type=int, default=26, metavar='N',
                    help='num classes for mask rcnn (default : 26)')
parser.add_argument('--mask_id', type=int, default=25, metavar='N',
                    help='number of wire id (default : 25)')
parser.add_argument('--mask_lr', type=float, default=1e-4, metavar='LR',
                    help='learning rate (default: 0.0001)')
parser.add_argument('--mask_iters', type=int, default=20000, metavar='N',
                    help='number of iterations to train (default: 20000)')
parser.add_argument('--mask_batch', type=int, default=4, metavar='N',
                    help='number of batch size in mask rcnn (default: 4)')
parser.add_argument('--mask_batch_per_images', type=int, default=512, metavar='N',
                    help='number of batch size per image (default: 512)')
parser.add_argument('--mask_checkpoint', type=int, default=2000, metavar='N',
...
parser.add_argument('--train_img_sizes', type=int, nargs='+', metavar='N',
                    help='train image sizes')  # --train_img_sizes 192 256 320 380
parser.add_argument('--train_img_size', type=int, default=320, metavar='N',
                    help='train image size (default: 320)')
parser.add_argument('--test_img_size', type=int, default=380, metavar='N',
                    help='test image size (default: 380)')
parser.add_argument('--dataset_maximum', type=int, default=100000, metavar='N',
...
parser.add_argument('--no_cast', action='store_true', default=False,
                    help='Autocasting off (default: False)')

                    
# log
parser.add_argument('--val_per_epochs', type=int, default=20, metavar='N',
                    help='validation per epoch (default: 20)')
parser.add_argument('--test_per_epochs', type=int, default=20, metavar='N',
                    help='test per epoch (default: 20)')
parser.add_argument('--log-interval', type=int, default=10, metavar='N',
                    help='how many batches to wait before logging training status')
parser.add_argument('--start_time', type=str, default="no_input", metavar='S',
...
def train_all(args):
    ...
    args.mask_output_dir = os.path.join(args.output_dir, "mask_rcnn", f"{args.start_time}")
    ...
    if not args.testing:
        if (round(args.AP, 3) + 0.4) > float(args.mask_path.split("_")[-1][:-3]): # 'args.AP' is made in 'mask_train.py'
            args.mask_path = args.mask_best_model
            mask_inference(args, renewal=True) # renewal : 기존의 배경 제거한 전선을 삭제하고 새 모델로 배경 제거 전선을 생성
