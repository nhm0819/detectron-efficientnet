...
class Labeller:
    def __init__(
        ...
        self.type_info = {"3": "flame", "12": "electric", "9": "indistinct"}
    ...
    def get_json_folder(self):
        sql_datapath = (
            f"select datapath from ln_weight_info where weightid={self.weight_id};"
        )
    ...
    def get_mask_feature(self, binary_mask_path):
        ...
        mask_feature = mask_feature.reshape(
            (1, self.nms_cnt, self.num_classes, 28, 28)
        )  # mask feature shape : (28, 28)
        mask_feature = mask_feature[
            0, 0, self.wire_class_num, :, :
        ]  # [first image, first index, wire_class(= 1 dim), :28, :28] --> [28, 28]
    ...
    def make_mask_image(self, mask_feature, bbox, threshold=0.5):
        ...
        cropped_mask = cv2.resize(
            mask_feature, (x2 - x1, y2 - y1), interpolation=cv2.INTER_LINEAR
        ).astype(np.float32)
    ...
    def get_points(self, mask_image):
        ...
        contours, hierarchy = cv2.findContours(
            src, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
        )  # RETR_CCOMP
        contour = max(contours, key=lambda x: len(x))
        epsilon = self.epsilon_rate * cv2.arcLength(
            contour, True
        )  # alpha is the rate of th arc length
    ...
    def autolabel(self, binary_mask_path, RIP_path):
        ...
        try:
            mask_image = self.make_mask_image(
                self.get_mask_feature(binary_mask_path), bbox
            )
        except:
    ...
    def labelling(self):
        ...
        for idx, labelling_item in enumerate(self.labelling_list):
            ...
            if wire_type is not None:  # 'flame' or 'electric' or 'indistinct'
                # labelling
                json_data = self.autolabel(
                    binary_mask_path=binary_mask_path, RIP_path=RIP_path
                )
                ...
                print(
                    f"idx : {idx}, specimenid : {labelling_item[0]}, json_path : {json_path}"
                )
                ...
                # update database
                sql_labelling_list = (
                    f"UPDATE ln_weight_item SET jsonpath='{json_path}' WHERE"
...
if __name__ == "__main__":
    ...
    ##############
    with open(
        "infer_data\\G3FAhN3gR140bKp6HFYMxjxy2bgxmeMA_1643084025005_1_0_mask_RIP.txt"
    ) as f:
        bbox_list = f.readlines()
        bbox = bbox_list[0].split(",")[:4]
        bbox = np.array(bbox, dtype=float).astype(int)
    ...
    np_mask = np.fromfile(
        "infer_data\\G3FAhN3gR140bKp6HFYMxjxy2bgxmeMA_1643084025005_1_0_binaryMaskData",
        dtype=np.float32,
    )
    np_mask = np_mask.reshape((1, 3, 27, 28, 28))
    mask_feature = np_mask[
        0, 0, 26, :, :
    ]  # [num_image, first_idx, wire_class, :28, :28]
