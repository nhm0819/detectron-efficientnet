...
def before_request():
    ...
    if request.remote_addr not in [
        os.getenv("KESCO-DB-HOST"),
        os.getenv("KESCO-INFERENCE"),
        "127.0.0.1",
    ]:
        abort(403)  # Forbidden
    ...
    g.db = mysql.connector.connect(
        host=os.getenv("KESCO-DB-HOST"),
        port=os.getenv("KESCO-DB-PORT"),
        database=os.getenv("KESCO-DB-NAME"),
        user=os.getenv("KESCO-DB-USER"),
...
def train():
    ...
    sql_training_list = (
        f"select weightid, trainingYN, pid from ln_weight_info where trainingYN='Y';"
    )
    ...
    clf_orig = os.path.join(
        g.FILE_STORAGE_PATH, "weights", "efficientnet_over_current_acc_98.55.pt"
    )
    ...
    sql_weight_info = (
        f"select createtm, datapath from ln_weight_info where weightid={weight_id};"
    )
    ...
    check_training_weights = os.path.join(
        g.TRAIN_RESULTS_PATH, "efficientnet", create_time, "*.weights"
    )
    ...
    # Database Update
    sql_update_training = (
        f"UPDATE ln_weight_info SET classificationfileurl = '{clf_weight_path}', "
...
def training_stop(
    session_id="0", weight_id=-1, training_error=False, terminate_mode=False
):
    ...
    if training_error:  #
        ...
        sql_training_proc = (
            f"select createtm, pid from ln_weight_info where weightid={weight_id};"
        )
        ...
        sql_update_training = (
            f"UPDATE ln_weight_info SET trainingYN='P' WHERE weightid={weight_id};"
        )
        cursor.execute(sql_update_training)
        db.commit()
        ...
        sql_training_proc = (
            f"select createtm, pid from ln_weight_info where trainingYN='Y';"
        )
        cursor.execute(sql_training_proc)
        training_list = cursor.fetchall()
        ...
        if terminate_mode:
            sql_update_training = (
                f"UPDATE ln_weight_info SET trainingYN='N' WHERE trainingYN='Y';"
            )
            cursor.execute(sql_update_training)
            db.commit()
        else:
            sql_update_training = (
                f"UPDATE ln_weight_info SET trainingYN='P' WHERE trainingYN='Y';"
            )
            cursor.execute(sql_update_training)
            db.commit()
    ...
    if os.path.isdir(mask_folder):
        ...
        if os.path.isfile(mask_log):
            ...
            if os.path.isfile(clf_log):
                ...
                percentage = str(
                    int(latest_epoch / 200 * 100 * 0.5) + 50
                )  # total epochs = 200, +50% : after mask rcnn complete
                return percentage
            ...
            if len(iters) != 0:
                latest_iter = iters[-1]
            percentage = str(
                int(latest_iter / 20000 * 100 * 0.5) + 1
