...
def read_fastcgi_record(stream):
    ...
    fcgi_ver, reqtype, req_id, content_size, padding_len, _ = struct.unpack(
        ">BBHHBB", data
    )
...
def read_fastcgi_get_values(stream, req_id, content):
    ...
    send_response(
        stream, req_id, FCGI_GET_VALUES_RESULT, write_fastcgi_keyvalue_pairs(response)
    )
...
def log(txt):
    ...
    if log_file:
        with open(log_file, "a+", encoding="utf-8") as f:
            ...
            f.write(
                "%s: %s%s"
                % (datetime.datetime.now(), txt, "" if txt.endswith("\n") else "\n")
            )
...
def start_file_watcher(path, restart_regex):
    ...
    def enum_changes(path):
        ...
        while True:
            ...
            if ret_code:
                ...
                while True:
                    fni = ctypes.cast(
                        cur_pointer, ctypes.POINTER(FILE_NOTIFY_INFORMATION)
                    )
    ...
    log(
        "wfastcgi.py will restart when files in %s are changed: %s"
        % (path, restart_regex)
    )
    ...
    def watcher(path, restart):
        for filename in enum_changes(path):
            if not filename:
                ...
                log(
                    "wfastcgi.py exiting because %s has changed, matching %s"
...
def read_wsgi_handler(physical_path):
    ...
    for path in (v for k, v in env.items() if k.lower() == "pythonpath"):
        ...
        expanded_path = re.sub("%(\\w+?)%", lambda m: os.getenv(m.group(1), ""), path)
...
class handle_response(object):
    ...
    def __enter__(self):
        ...
        record.params["wsgi.url_scheme"] = (
            "https" if record.params.get("HTTPS", "").lower() == "on" else "http"
        )
        record.params["wsgi.multiprocess"] = True
        record.params["wsgi.multithread"] = False
        record.params["wsgi.run_once"] = False

        self.physical_path = record.params.get(
            "APPL_PHYSICAL_PATH", os.path.dirname(__file__)
        )
        ...
        if "HTTP_X_ORIGINAL_URL" in record.params:
            ...
            record.params["wsgi.path_info"] = record.params[
                "wfastcgi.http_x_original_url"
            ]
        ...
        record.params["wsgi.path_info"] = record.params["wsgi.path_info"].partition(
            wsgi_encode("?")
        )[0]
    ...
    def send(self, resp_type, content, streaming=True):
        ...
        return send_response(
            self.stream, self.record.req_id, resp_type, content, streaming
        )
...
def main():
    ...
    try:
        ...
        while True:
            ...
            with handle_response(
                fcgi_stream, record, output.getvalue, errors.getvalue
            ) as response:
                if not initialized:
                    ...
                    start_file_watcher(
                        response.physical_path, env.get("WSGI_RESTART_FILE_REGEX")
                    )
                    ...
                    if ptvsd_secret:
                        ptvsd_address = (
                            env.get("WSGI_PTVSD_ADDRESS") or "localhost:5678"
                        ).split(":", 2)
                        try:
                            ...
                            log(
                                '"%s" is not a valid port number for debugging'
                                % ptvsd_address[1]
                            )
                            ptvsd_port = 0
                ...
                if "SCRIPT_NAME" in os.environ and record.params[
                    "PATH_INFO"
                ].lower().startswith(os.environ["SCRIPT_NAME"].lower()):
...
def _run_appcmd(args):
    ...
    if len(sys.argv) > 1 and os.path.isfile(sys.argv[1]):
        ...
        appcmd = [
            os.path.join(os.getenv("SystemRoot"), "system32", "inetsrv", "appcmd.exe")
        ]

    if not os.path.isfile(appcmd[0]):
        print(
            "IIS configuration tool appcmd.exe was not found at",
            appcmd,
            file=sys.stderr,
        )
        return -1
...
def enable():
    ...
    if res == 0:
        print(
            '"%s|%s" can now be used as a FastCGI script processor'
            % (executable, quoted_file)
        )
    return res
...
def disable():
    ...
    if res == 0:
        print(
            '"%s|%s" is no longer registered for use with FastCGI'
            % (executable, quoted_file)
        )
    return res
