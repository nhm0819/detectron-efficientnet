    ...
    def forward(self, outputs, original_image, mask_id=25):
        ...
        for idx, output in enumerate(outputs):
            ...
            mask = torch.as_tensor(
                instances.pred_masks[wire_idx], device="cpu"
            )  # .clone().detach()
            ...
            mask_gt = torch.gt(
                mask, 0
            )  # np.greater(mask, 0)  # get only non-zero positive pixels/labels
            mask_exp = mask_gt.unsqueeze(
                -1
            )  # np.expand_dims(mask, axis=-1)  # (H, W) -> (H, W, 1)
            mask_3c = torch.cat(
                (mask_exp, mask_exp, mask_exp), axis=-1
            )  # np.concatenate((mask, mask, mask), axis=-1)
            ...
            mask_image = torch.where(
                mask_mul == 0, torch.tensor(255, dtype=torch.uint8), mask_mul
            )
            # mask_image[where_0] = 255

            try:
                x1, y1, x2, y2 = (
                    instances.pred_boxes.tensor[0].type(torch.int64).squeeze()
...
class CustomPredictor(DefaultPredictor):
    ...
    def __call__(self, original_image):
        ...
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            masks, wire_probs = self.post_process(
                predictions, original_image, mask_id=self.mask_id
            )
