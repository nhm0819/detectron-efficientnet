        ...
        for idx, line in enumerate(tepoch):
            ...
            for i, image_path in enumerate(image_paths):
                ...
                if "original" in image_path:
                    ...
                    mask_image_path = mask_image_path.replace("original", "original\\masked_image")

                else:
                    file_name = os.path.basename(image_path)
                    mask_image_path = os.path.join(args.mask_folder,
                                                   file_name)

                if not os.path.isdir(os.path.dirname(mask_image_path)):
...
def mask_inference(args, renewal=False):
    ...
    train_loader = DataLoader(train_dataset,
                              ...
                              collate_fn=make_batch, # collate_fn : dataset을 batch로 불러들이기 위한 함수
                              pin_memory=True)

    inference(args, predictor, train_loader, renewal=renewal, is_train="train") # renewal : 배경 제거된 이미지를 새 모델로 교체
    ...
    inference(args, predictor, test_loader, renewal=renewal, is_train="test")  # renewal : 배경 제거된 이미지를 새 모델로 교체
