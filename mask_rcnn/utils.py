...
def save_plot(output_folder, experiment_metrics):
    ...
    ax2.plot(
        # [x['iteration'] for x in experiment_metrics if 'validation_loss' in x],
        [x['segm/AP'] for x in experiment_metrics if 'segm/AP' in x], color=color, label="AP")
...
def select_best(args):
    ...
    best_model = os.path.join(output_dir, f"model_{((best_idx+1)*args.mask_checkpoint-1):07d}.pth")
