...
def train_fn(
    ...
    # training for epochs
    for epoch in range(
        args.epoch_per_stage * (stage - 1) + 1, args.epoch_per_stage * stage + 1
    ):
        model.train()
        ...
        if epoch % args.test_per_epochs == 0:
            ...
            val_loss, val_accuracy = test_epoch(
                args, model, device, val_loader, criterion, LOGGER
            )
            if val_accuracy > best_accuracy:
                ...
                torch.save(
                    ...
                    os.path.join(
                        args.eff_output_dir,
                        f"efficientnet_b4_acc_{best_accuracy:.2f}.pt",
                    ),
                )
                LOGGER.info(
                    f"Save the best acc model, loss = {val_loss:.2f}, acc = {best_accuracy:.2f}"
                )

            # if valid_loss < best_loss:
            #     best_loss = valid_loss
            #     torch.save(model.state_dict(),
            #                os.path.join(args.eff_output_dir, f'efficientnet_b4_stage_{stage}_best_loss.pt'))
...
def train_epoch(
    ...
    end_time = time.time()
    LOGGER.info(
        f"Train Mean Loss : {mean_loss:.8f},\tTrain Accuracy: {accuracy:.2f},"
